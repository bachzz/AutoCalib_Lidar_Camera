...
class AutoCalibration():
    def __init__(self, image, pointcloud, init_params, config, max_iters = 300):
    ...
    def project_pointcloud_to_image(self, ext_vec):
        return self.utils.project_pointcloud_to_image(self.pointcloud, self.image_bgr, ext_vec, self.cam_K) 
    ...
    def compute_hist(self, ext_vec):
        ...
        for point in self.pointcloud:
            ...
            if (point_dist / DIST_THRES) < 1: # avoid points in inifite
                ...
                ### validate point
                if not self.utils.validate_image_point(p_C, self.image):
                    continue
                # breakpoint()
                intensity_bin = int(pi / self.bin_fraction)
                gray_bin = int(self.image[p_C[1], p_C[0]] / self.bin_fraction)
