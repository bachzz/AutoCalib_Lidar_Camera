...
class AutoCalibration():
    def __init__(self, images, images_depth, pointclouds, init_params, config, max_iters = 300, gt_params=None, params_grid=None):
    ...
    def compute_hist(self, ext_vec):
        ...
        for i in range(self.N):
            ...
            mask_L = ((self.pointclouds[i][:, 0] == 0) & (self.pointclouds[i][:,1] == 0) & (self.pointclouds[i][:, 2] == 0))
            
            # pointcloud_dists = np.apply_along_axis(np.linalg.norm, 0, self.pointclouds[i].T[:3,:])
            pointcloud_dists = np.sqrt((self.pointclouds[i].T[:3,:]**2).sum(axis=0))
            p_C_intensity = np.append(p_C_, np.array([pointcloud_dists]), axis=0)
            p_C_intensity_masked = np.where(mask_C, p_C_intensity, np.nan)
            p_C_intensity_masked = np.where(~mask_L, p_C_intensity_masked, np.nan)
